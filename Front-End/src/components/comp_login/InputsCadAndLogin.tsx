    ...
    <div className="flex flex-col justify-center items-center w-full gap-2 mb-3">
      {!props.NoUserName && (
        <>
          ...
          <input
            className="w-4/5 p-2 border border-blue-900 rounded-md bg-zinc-800/40 text-white"
      ...
      <h6>E-mail</h6>
      <input
        className="w-4/5 mb-2 p-2 border border-blue-900 rounded-md bg-zinc-800/40 text-white"
      ...
      {!props.NoForgorPassword && (
        <Link href="/forgot-password" className="flex justify-center items-center gap-2">
          <p className="text-sm text-blue-900">Esqueceu sua senha?</p>
        </Link>
      )}
    </div>
  );
